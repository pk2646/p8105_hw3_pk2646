...
accel_df %>% 
  group_by(week, day) %>% 
  summarize(mean_activity = mean(activity_min)) %>% 
  pivot_wider(
    names_from = day,
    values_from = mean_activity
  ) %>% 
  relocate(week, Monday, Tuesday, Wednesday, Thursday, Friday, Saturday) %>% 
  knitr::kable()
```

On average it seems like the mean minutes is higher on Fridays. 
